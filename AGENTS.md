...
6. **Icon controls**: icon-only controls always use real SVG/icon components in stable square or round boxes. Never use text glyphs such as `✓`, `⌄`, `⋯`, `+`, or arrows as UI icons. **The line is whether the character is the content or a substitute for a drawing:** a letter naming its own format is a label and is allowed (B for bold, I for italic, S for strikethrough, each rendered in the format it names, each with an aria-label, sized from the type scale). A character standing in for an icon is banned, which is every case above. Icons default to muted unless they communicate state. Gold is reserved for active state icons, new/fresh state icons, and hover states.
